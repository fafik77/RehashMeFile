/**
 ...
 {
 	memset(context, 0x00, sizeof(Md5Context)); //zero out
 ...
 void md5Final(Md5Context *context, uint8_t *digest)
 {
	...
	uint64_t totalSize= context->totalSize * 8;
	...
	if(digest != NULL)
	{ //fafik: for some reason we have to invert the order to match (from BigIndian to Little)
		int t_i= 16;
		uint8_t* out= digest+ 16;
		uint8_t* in= context->digest;
		while(t_i){
			--t_i;
			--out;
			(*out)= (*in);
