...
void Hash::MD5::ProcessBlock(std::vector<uint32_t>& block)
{
...
void MD5::update(const uint8_t* inputBuffer, size_t inputLength) {
	...
	for (; inputPos + 63 < inputLength; inputPos += 64) { //work with full blocks
		memcpy(&input.front(), inputBuffer + inputPos, 64); //use raw memcopy as its more efficient then loop byte copy
...
std::string MD5::FinalizeHash() {
	finalize();
	return CurrentHashAsString();
}
...
std::string MD5::stringToHash(const std::string& input) {
	std::vector<uint8_t> digest = MD5::digestString(input);
	return DigestHashAsString(digest);
}

std::string Hash::MD5::CurrentHashAsString() const
{
	return DigestHashAsString(this->digest);
}

std::string Hash::MD5::DigestHashAsString(const std::vector<uint8_t>& digest)
{
	std::stringstream ss;
