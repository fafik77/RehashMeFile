...
namespace Hash {
	...
	private:
		 ...
		 */
		void finalize();
		void ProcessBlock(std::vector<uint32_t>& block);
	public:
		MD5();
		/**
		 * @brief Returns the current hash value as a string.
		 * @return A std::string containing the current hash value.
		 */
		std::string CurrentHashAsString() const;
		/**
		 * @brief Finalizes the hashing and returns the hash value as a string.
		 * @return A std::string containing the current hash value.
		 */
		std::string FinalizeHash();
		/**
		 * @brief Returns the hash value of the digest as a string.
		 * @param digest the digest
		 * @return A std::string containing the digest hash value.
		 */
		static std::string DigestHashAsString(const std::vector<uint8_t>& digest);
		/**
	...
	public:
		 ...
		 */
		static std::string stringToHash(const std::string& input);
		/**
